...
const SPEC_INSTRUCTION = [
  ...
  'For ARTICULATED robots (industrial arms, SCARA, delta, multi-joint manipulators): give EACH rotary joint node a "swing" on its joint axis (amp 0.2-0.5 rad, freq 0.4-1.0) so the robot visibly ARTICULATES when driven. Nest links as children so motion propagates down the arm. Do not leave an articulated robot fully static.',
